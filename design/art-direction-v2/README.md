# Art direction v2 — proof of concept (Stage 1, gate §1.1)
...
- **§1.2**: **Quarky v2: yes.**
- Written into `.claude/skills/art-direction/SKILL.md` (art direction v2 + v1 → v2 log) and [ADR-0001](../../docs/decisions/ADR-0001-art-direction-v2.md). This page stays as the reference board; the Android level screen (`android/game`) is the first full "B + C background" implementation.

## Data source and compromises

- Everything comes from `quantique_tunnel_1.json` as exported at the time (schema v2; today `content/levels/quantique/quantique_tunnel_1.json`, schema v3, same geometry): launcher (0.1, 0.5), portal (0.86, 0.5, r 0.05), a vertical partition at x 0.5 (`wTop` / `wBot`) with a barrier window 0.20 high (threshold 0.6 ± 0.45, period 1.0 s, shown in real time), and the file's 3 Photons (p1 oscillates in y).
...
The on-screen labels of this page (mode buttons such as **Boucle**, **Succès**, **Échec**) are in French: the page is a frozen design artifact and is not translated. Player-facing text of the game lives in the app resources (English + French).
