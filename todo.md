...
- [ ] **`quantification` isn't distinct:** discrete power "notches" (`choice` values) are already used by every other level, so the concept has no mechanic of its own. *Fix approved (rung-lock, ADR-0008); implementation in S5.*
