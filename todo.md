...
- [ ] **3-star system not actually implemented:** 6/7 levels have **1 Photon** (only `intrication` has 3), so 3 stars can't be earned. `gameplay-mechanics` requires 3 per level.
- [ ] **Degenerate in-flight taps:** the best solution for `dualite` and `intrication` is `tap_time = 0.0`. Tapping at launch makes the tap equivalent to a pre-launch setting, which defeats the rule "every concept depends on time during flight". The validator must reject or down-rank solutions where the tap happens before a minimum flight time.
- [ ] **Brittle / over-tight levels:** `tunnel` has 2 solutions in the whole grid, `quantification` has 9 and `dualite` has 7. `spin` restricts the angle to `[-12, -11]` (the solution is effectively hard-coded). We need a *tolerance* metric, not just a solvable/not-solvable answer.
...
- [ ] **Sticky collisions:** handlers are called every step while inside an obstacle's radius. Reflections can re-trigger and jitter (the "trapped ricochet" bug was patched by moving walls around, not by fixing the root cause). Only reflect when `dot(vel, normal) < 0`.
- [ ] **Stale references:** `concepts.py` / `generator.py` still say "8 concepts", `concepts.py` documents a `trap` event left over from the removed Décohérence concept, and `gameplay-mechanics` says "ex. 8 concepts pour le Quantique".
- [ ] **No tests, no `pyproject.toml`, no level schema version, no README.** The JSON export leaks `param_space` and the full reference solution into the game payload with no versioning.
- [ ] **No agents exist** (`.claude/agents/` is missing), no `.claude/settings.json`, and no CI.
